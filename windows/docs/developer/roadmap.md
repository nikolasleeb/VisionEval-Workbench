...
5. **Release maturity:** after the unsigned ARM64 v1, add signing/notarization, validate additional platforms, and keep the pinned runtime and PlanRVA regression review current.
...
- **Current limitation:** v1 is an unsigned Apple Silicon release. Intel Mac and Windows are unvalidated.
- **Dependencies:** Apple credentials, platform-specific runtime validation, approved image, and compatibility manifests.
- **Acceptance:** signed/notarized/stapled ARM64 DMG plus equivalent tested packages for any newly supported platform; verified checksums and clean-machine smoke tests.
