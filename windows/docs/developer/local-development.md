...
The only supported execution adapter today is Linux ARM64 Docker. Intel Mac and Windows types exist as architectural placeholders, not supported runtimes.
...
```bash
cd VisionEvalWorkbench
VISIONEVAL_WORKSPACE_ROOT="$PWD/.development-workspace" python3 backend/main.py
```

Open `http://127.0.0.1:3000`. Source mode resolves user documentation from `docs/user/`; packaged mode resolves it from the PyInstaller resource bundle.
...
`packaging/build-backend.sh` creates the platform-specific sidecar under `desktop/src-tauri/binaries/`. `packaging/workbench-backend.spec` bundles the web UI, R helpers, metadata catalogs, and the canonical user guide. The Tauri configuration produces an ARM64 DMG.
