...
`docs/user/documentation.json` declares the canonical documentation version. On sidecar startup, `DocumentationService` installs the guide into `Documentation/Workbench User Guide/`, writes `Documentation/README.md`, and records every managed path and SHA-256 in `Documentation/.workbench-documentation.json`.
