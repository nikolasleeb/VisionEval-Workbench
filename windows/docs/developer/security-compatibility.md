...
The current supported runtime is Apple Silicon macOS with Docker ARM64. Runtime-profile schemas reserve Intel Mac and Windows adapters, but those combinations must be labeled unsupported until implemented and tested. Model compatibility requires a complete runnable folder with `visioneval.cnf`, `scripts/run_model.R`, `defs/`, and `inputs/`.
