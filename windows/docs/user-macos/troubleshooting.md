...
The data packages are platform-neutral, but the released macOS app and runtime are Apple Silicon artifacts. Do not load the macOS DMG or `linux/arm64` runtime on a typical x64 Windows machine. Use the separate Windows installer and follow the [Windows Installation and Runtime](https://github.com/nikolasleeb/VisionEval-Workbench/wiki/Windows-Installation-and-Runtime) guide.
