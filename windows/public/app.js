...
function applyLargeDialogLayout() {
  ...
  Object.entries(LARGE_DIALOG_SIZES).forEach(([id, defaults]) => {
...
new MutationObserver((records) => { if (records.some((record) => record.attributeName === "open")) requestAnimationFrame(syncWorkbenchViewport); }).observe(document.body, {subtree:true, attributes:true, attributeFilter:["open"]});
Object.keys(LARGE_DIALOG_SIZES).forEach((id) => document.getElementById(id)?.classList.add("workbench-large-dialog"));
function resizeSettingsDialog(width, height) {
  const minimumWidth = Math.min(640, Math.max(0, workbenchViewportBounds.width - 2 * WORKBENCH_DIALOG_GAP)), minimumHeight = Math.min(480, Math.max(0, workbenchViewportBounds.height - 2 * WORKBENCH_DIALOG_GAP));
...
function columnDetails(csv, column) {
  ...
  if (protectedColumn(column)) return {kind:"protected",bulkEditable:false,directEditable:false,type:csv?.columnTypes?.[column]||""};
  if (numeric) return {kind:"numeric",bulkEditable:true,directEditable:true,type:csv?.columnTypes?.[column]||"number"};
  if (nonblank.length >= 2 && nonblank.length <= 50) return {kind:"categorical",bulkEditable:true,directEditable:true,type:csv?.columnTypes?.[column]||"character",options:values};
  return {kind:"text",bulkEditable:false,directEditable:false,type:csv?.columnTypes?.[column]||"character"};
}
...
function renderEditorShareGroup() {
  const group = selectedEditorGroup(), container = $("editorShareGroup"); container.hidden = !group;
  if (!group) { container.innerHTML = ""; return; }
  const row = scopedEditorRow(), fields = group.members.map((member) => { const index = state.csv.columns.indexOf(member), value = row[index] ?? "", details = columnDetails(state.csv, member); return `<label>${escapeHtml(member)}<input type="number" step="any" data-editor-group-member="${escapeHtml(member)}" value="${escapeHtml(value)}" aria-describedby="editorGroupSummary"><small class="muted">${escapeHtml(details.guidance || "Proportion · valid range 0–1")}</small></label>`; }).join("");
  container.innerHTML = `<strong>Linked share composition</strong><p class="muted">Enter the complete vector. It will be applied to every matched row; other shares are not redistributed.</p><div class="share-group-fields">${fields}</div><p id="editorGroupSummary" class="share-group-summary" role="status"></p>`;
  container.querySelectorAll("input").forEach((input) => input.addEventListener("input", () => updateShareGroupSummary(container, group)));
  updateShareGroupSummary(container, group);
}
...
function updateShareGroupSummary(container, group) {
  const mapping = shareGroupValues(container, container.id.startsWith("batch") ? "batch" : "editor"), values = Object.values(mapping), numbers = values.map(Number), blank = values.map((value) => !String(value).trim()), total = numbers.reduce((sum, value) => sum + (Number.isFinite(value) ? value : 0), 0), target = Number(group.target ?? 1), tolerance = Number(group.tolerance ?? .000001), csv = group.csv || state.csv, rangeValid = Object.entries(mapping).every(([member, value]) => !String(value).trim() || !numericValueError(columnDetails(csv, member), value)), optionalBlank = Boolean(group.optional && blank.every(Boolean)), valid = optionalBlank || !blank.some(Boolean) && numbers.every(Number.isFinite) && rangeValid && (group.rule === "sum_at_most" ? total <= target + tolerance : Math.abs(total - target) <= tolerance), remainder = group.remainderLabel ? ` · ${group.remainderLabel}: ${Math.max(0, target - total).toFixed(6)}` : "", summary = container.querySelector(".share-group-summary");
  summary.textContent = optionalBlank ? "Optional group is blank · valid" : `Total: ${total.toFixed(6)}${remainder} · ${valid ? "valid" : "not valid"}`;
  summary.classList.toggle("invalid", !valid); return valid;
}
function syncEditorEditMode(preferredValue = null) {
  ...
  renderEditorShareGroup(); syncEditorColumnSelectAll(); updateFileDraftGuidance();
}
...
function syncBatchEditMode(preferredValue = null) {
  ...
  if (categorical) { $("batchOperation").value = "set"; const options = selection?.options || [], prior = preferredValue === null ? $("batchCategoryValue").value : preferredValue; $("batchCategoryValue").innerHTML = options.map((value) => `<option value="${escapeHtml(value)}">${escapeHtml(categoricalLabel(value))}</option>`).join(""); $("batchCategoryValue").value = options.includes(prior) ? prior : options[0] ?? ""; }
  renderBatchShareGroup(); syncBatchSelectAll(); renderBatchCompatibility(); updateBatchDraftGuidance();
}
