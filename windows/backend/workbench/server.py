            ...
            try:
                if parsed.path == "/api/health":
                    ...
                    send_json(self, {
                        "filename": filename,
                        "columns": columns,
                        "columnTypes": application.explore.input_column_types(filename, columns),
                        "columnMetadata": metadata,
                        "validationGroups": application.explore.validation_groups(filename),
                        "rows": rows[1:],
                        "overlay": overlay,
                    })
            ...
            try:
                ...
                if parsed.path == "/api/assets/bundled/planrva/install":
                    ...
                    metadata = application.explore.input_column_metadata(
                        filename, payload.get("columns") or [], source_rows[1:], current_rows[1:]
                    )
                    application.explore.validate_input_rows(
                        filename, payload.get("columns") or [], payload.get("rows") or [], current_rows[1:], metadata
                    )
                    ...
                    send_json(self, application.workspace.save_overlay(
                        project_id, variation_id, filename, content,
