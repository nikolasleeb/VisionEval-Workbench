...
class RuntimeTests(unittest.TestCase):
    ...
    def test_verification_rejects_non_amd64_image(self):
        class WrongArchitectureRunner(FakeRunner):
            def __call__(self, command, **kwargs):
                result = super().__call__(command, **kwargs)
                if "image" in command and "inspect" in command and "{{json .Config.Labels}}" in command:
                    labels = json.loads(result.stdout)
                    labels["org.opencontainers.image.architecture"] = "arm64"
                    return subprocess.CompletedProcess(command, 0, json.dumps(labels), "")
                return result

        with tempfile.TemporaryDirectory() as directory, patch.object(RuntimeManager, "_dispatch_loop", return_value=None), patch("backend.workbench.runtime.find_docker_executable", return_value="/docker"):
            runtime = RuntimeManager(Workspace(directory), runner=WrongArchitectureRunner())
            with self.assertRaisesRegex(WorkspaceError, "arm64"):
                runtime.verify_runtime()
    ...
    def test_intel_host_selects_amd64_image_and_platform(self):
        with patch("backend.workbench.runtime.platform.system", return_value="Darwin"), patch("backend.workbench.runtime.platform.machine", return_value="x86_64"), patch("backend.workbench.runtime.macos_running_under_rosetta", return_value=False):
            self.assertEqual(docker_platform(), "linux/amd64")
            self.assertEqual(local_runtime_image(), AMD64_LOCAL_IMAGE)
            self.assertTrue(docker_host_supported())

    def test_wrong_macos_architecture_is_rejected(self):
        with patch("backend.workbench.runtime.platform.system", return_value="Darwin"), patch("backend.workbench.runtime.platform.machine", return_value="arm64"), patch("backend.workbench.runtime.macos_running_under_rosetta", return_value=False):
            self.assertEqual(docker_platform(), "unsupported/arm64")
            self.assertFalse(docker_host_supported())

    def test_rosetta_host_is_rejected(self):
        with patch("backend.workbench.runtime.platform.system", return_value="Darwin"), patch("backend.workbench.runtime.platform.machine", return_value="x86_64"), patch("backend.workbench.runtime.macos_running_under_rosetta", return_value=True):
            self.assertFalse(docker_host_supported())

    def test_intel_alias_matches_the_published_runtime_contract(self):
        self.assertEqual(
            AMD64_LOCAL_IMAGE,
        ...
        class InstallRunner(FakeRunner):
            def __call__(self, command, **kwargs):
                ...
                if "image" in command and "inspect" in command and "RepoDigests" in " ".join(command):
                    return subprocess.CompletedProcess(command, 0, f"{published_reference}\n", "")
                return subprocess.CompletedProcess(command, 0, "{}", "")
    ...
    def test_managed_runtime_install_is_rejected_outside_intel_macos(self):
        with tempfile.TemporaryDirectory() as directory, \
            ...
            runtime = RuntimeManager(Workspace(directory), runner=FakeRunner())
            with self.assertRaisesRegex(WorkspaceError, "Intel macOS only"):
