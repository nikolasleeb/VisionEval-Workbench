...
async function verifyAndSaveRuntime() {
  ...
  if (window.__TAURI_INTERNALS__?.invoke) {
    const prior = (state.desktop?.runtimeProfiles || []).find((item) => item.id === state.desktop?.activeRuntimeProfileId);
    await window.__TAURI_INTERNALS__.invoke("save_runtime_profile", {profile:{
      id:prior?.adapter===(native?"native":"docker")?prior.id:"", name:native?"Windows VE_Runtime":"Intel Mac Docker", adapter:native?"native":"docker", platform:result.platform || (native?"windows":"darwin"), architecture:result.architecture || (native?"amd64":"x86_64"),
...
async function openSettings(page="settingsWorkspace") {
  ...
  renderUpdateSettings(state.data?.updates||workspaceSettings.updateChecks);
...
function updateSettingsValue(){return{automatic:$("automaticUpdateChecks").checked,sources:{visioneval:$("updateSourceVisionEval").checked,runtimeImage:$("updateSourceRuntime").checked,workbench:$("updateSourceWorkbench").checked}}}
function renderUpdateSettings(payload=state.data?.updates){if(!payload)return;const settings=state.data?.workspaceSettings?.updateChecks||payload;$("automaticUpdateChecks").checked=Boolean(settings.automatic);$("updateSourceVisionEval").checked=settings.sources?.visioneval!==false;$("updateSourceRuntime").checked=settings.sources?.runtimeImage!==false;$("updateSourceWorkbench").checked=settings.sources?.workbench!==false;$("updatesLastChecked").textContent=payload.lastCheckedAt?`Last checked ${formatTime(payload.lastCheckedAt)}.`:"Never checked.";const labels={workbench:"VisionEval Workbench",runtimeImage:"Workbench runtime image",visioneval:"VisionEval"},statusLabels={current:"Up to date",update_available:"Update available",unavailable:"Unable to check",not_selected:"Not selected",not_checked:"Not checked"};$("updateStatusList").innerHTML=["workbench","runtimeImage","visioneval"].map(source=>{const item=payload.statuses?.[source]||{status:"not_checked",message:"Not checked."},install=source==="runtimeImage"&&item.status==="update_available"&&item.runtimeProfile,restore=source==="runtimeImage"&&state.data?.runtime?.runtimeProfiles?.previous;return `<article class="update-status-card ${escapeHtml(item.status)}"><div class="update-status-heading"><strong>${labels[source]} · ${statusLabels[item.status]||"Not checked"}</strong><div>${item.url?`<button type="button" class="secondary" data-update-url="${escapeHtml(item.url)}">View release</button>`:""}${install?`<button type="button" data-install-runtime-update>Install Runtime Update</button>`:""}${restore?`<button type="button" class="secondary" data-restore-runtime>Restore Previous Runtime</button>`:""}</div></div><p>${escapeHtml(item.message||"")}</p>${item.installedVersion||item.availableVersion?`<small>Installed: ${escapeHtml(item.installedVersion||"Unknown")} · Available: ${escapeHtml(item.availableVersion||"Unknown")}</small>`:""}</article>`}).join("")}
async function checkUpdatesNow(){const button=$("checkUpdatesNow");setBusy(button,true,"Checking…");try{const updateChecks=updateSettingsValue();await post("/api/settings",{updateChecks});const payload=await post("/api/updates/check",{sources:Object.entries(updateChecks.sources).filter(([,value])=>value).map(([key])=>key)});state.data.updates=payload;state.data.workspaceSettings.updateChecks={...updateChecks,lastCheckedAt:payload.lastCheckedAt,statuses:payload.statuses};renderUpdateSettings(payload)}catch(error){notify(error.message||String(error),"error")}finally{setBusy(button,false)}}
async function activateRuntimeResult(result){const profile=result.activeProfile||{};await window.__TAURI_INTERNALS__.invoke("save_runtime_profile",{profile:{id:"",name:`${profile.visionEvalVersion||"Verified"} Docker runtime`,adapter:"docker",platform:"darwin",architecture:"x86_64",imageReference:profile.reference||result.image,imageDigest:profile.digest||result.digest,runtimeVersion:profile.visionEvalVersion||result.runtimeVersion,verified:true,verifiedAt:result.verifiedAt||new Date().toISOString(),verificationMessage:"Immutable digest and compatibility checks passed.",remoteStatus:"ghcr"}});state.desktop=await window.__TAURI_INTERNALS__.invoke("desktop_state")}
async function installRuntimeUpdate(button){const item=state.data?.updates?.statuses?.runtimeImage||{},profile=item.runtimeProfile||{},download=profile.downloadSizeBytes?humanBytes(profile.downloadSizeBytes):"an unreported amount",storage=profile.storageSizeBytes?humanBytes(profile.storageSizeBytes):"an unreported amount";if(!await confirmWorkbench(`Install ${item.availableVersion||"the compatible runtime update"}?\n\nDownload: ${download}\nApproximate local Docker storage: ${storage}\n\nWorkbench verifies it before activation and keeps the current runtime for rollback.`,{title:"Install runtime update?",confirmLabel:"Install Runtime Update",danger:false}))return;setBusy(button,true,"Installing…");try{const operation=await post("/api/runtime/install/start",{source:"update"}),result=await waitForRuntimeInstallation(operation.id);await activateRuntimeResult(result);await refreshState({quiet:true});renderUpdateSettings();notify("The verified runtime update is active.","success")}catch(error){notify(error.message||String(error),"error")}finally{setBusy(button,false)}}
async function restoreRuntime(button){if(!await confirmWorkbench("Restore the previous verified runtime?",{title:"Restore previous runtime?",confirmLabel:"Restore Runtime",danger:false}))return;setBusy(button,true,"Restoring…");try{const result=await post("/api/runtime/restore-previous",{});await activateRuntimeResult(result);await refreshState({quiet:true});renderUpdateSettings();notify("The previous verified runtime is active.","success")}catch(error){notify(error.message||String(error),"error")}finally{setBusy(button,false)}}
...
$("checkUpdatesNow").addEventListener("click",checkUpdatesNow);$("updateStatusList").addEventListener("click",event=>{const link=event.target.closest("[data-update-url]");if(link)return window.__TAURI_INTERNALS__.invoke("open_external_url",{url:link.dataset.updateUrl});const install=event.target.closest("[data-install-runtime-update]");if(install)return installRuntimeUpdate(install);const restore=event.target.closest("[data-restore-runtime]");if(restore)return restoreRuntime(restore)});
