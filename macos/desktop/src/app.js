...
const state = {
  ...
  mapOptions: [],
  hypercubeAnalysis: {projects:[],options:null,matrix:null,storage:null,selectedCases:[],view:"matrix",sort:"case",sliceValues:new Map(),matrixOperationId:"",discoveryOperationId:"",operationKind:"",operationStartedAt:0,pollTimer:null,discoveryResult:null,requestSnapshot:null},
  mapPayload: null,
  mapDirty: true,
  mapInputSignature: "",
...
async function loadCompareSelection(){
  ...
  if(!ids.length){state.comparisonIds=[];state.variables=[];renderVariableSelectors();return;}
  if(new Set(ids).size!==ids.length)return notify("Reference and comparison must be different results.","error");
  try{
    ...
    state.comparisonIds=ids;state.variables=payload.variables||[];state.lastComparison=null;state.comparisonScan=null;state.comparisonScanId="";state.exportFilterField="";state.exportFilterValues.clear();state.fullExportVariableKeys.clear();
    rememberComparisonPair(ids);resetCompareResults();renderVariableSelectors();await loadScanGeoOptions();syncSingleDatastoreControls();
  }catch(error){if(error.name!=="AbortError")notify(error.message,"error");}
}
[$("compareReference"),$("compareComparison")].forEach((control)=>control.addEventListener("change",loadCompareSelection));
...
async function exportArtifact(kind, override={}) {
  if(kind==="hypercube-analysis"){
    if(!state.hypercubeAnalysis.matrix)return notify("Update the Hypercube analysis before exporting it.","error");
...
function renderDashboardControls(){
  if(state.dashboardIds.length<2){$("dashboardVariableList").innerHTML=`<p class="muted">Choose two different results to build a percent-change chart.</p>`;$("generateDashboard").disabled=true;return;}
...
["dashboardReference","dashboardComparison"].forEach((id)=>$(id).addEventListener("change",()=>{setDashboardDirty();loadDashboardSelection();}));
...
async function loadHypercubeAnalysisProject(){
  ...
  try{
    ...
    renderHypercubeAnalysisProject();await loadHypercubeAnalysisGeography();
...
function renderHypercubeAnalysisProject(){
  ...
  axes.slice(2).forEach((axis)=>state.hypercubeAnalysis.sliceValues.set(axis.id,String(axis.values?.[0]??'')));renderHypercubeAnalysisSlices();renderHypercubeStorage();
}
...
function renderHypercubeStorage(){const report=state.hypercubeAnalysis.storage,card=$('hypercubeAnalysisStorage');if(!report){card.hidden=true;return;}card.hidden=false;card.innerHTML=`<div class="section-title"><div><h3>Hypercube result storage</h3><p class="muted">Datastores are authoritative. Full CSV exports are optional; comparison caches are disposable.</p></div>${report.reclaimableBytes?'<button id="removeHypercubeExports" class="danger">Remove Full CSV Exports…</button>':''}</div><div class="metric-grid">${metric('Completed cases',report.completedCases)}${metric('Datastores',humanBytes(report.datastoreBytes))}${metric('Full CSV exports',humanBytes(report.exportBytes))}${metric('Analysis cache',humanBytes(report.comparisonCache?.bytes||0))}${metric('Reclaimable',humanBytes(report.reclaimableBytes))}</div>`;$('removeHypercubeExports')?.addEventListener('click',removeHypercubeExports);}
...
function setHypercubeAnalysisLocked(locked,status=null){
  ...
  const controls=['hypercubeAnalysisProject','refreshHypercubeAnalysis','hypercubeAnalysisYear','hypercubeAnalysisTable','hypercubeAnalysisVariable','hypercubeAnalysisMetric','hypercubeAnalysisAggregation','hypercubeAnalysisGeography','hypercubeAnalysisLocations','hypercubeAnalysisXAxis','hypercubeAnalysisYAxis','swapHypercubeAxes','updateHypercubeAnalysis','discoverHypercubeOutputs'];
...
async function updateHypercubeAnalysis(){
  ...
  const requestPayload=hypercubeAnalysisRequest();if(!requestPayload.projectId||!requestPayload.table||!requestPayload.variable||!requestPayload.year)return notify('Choose a Hypercube output first.','error');
...
function renderHypercubeAnalysis(){const payload=state.hypercubeAnalysis.matrix;if(!payload)return;const item=(state.hypercubeAnalysis.options?.variables||[]).find((entry)=>entry.table===payload.table&&entry.name===payload.variable);$('hypercubeAnalysisTitle').textContent=`${payload.table} / ${payload.variable}`;$('hypercubeAnalysisSubtitle').textContent=`${hypercubeMetricLabel()} · ${item?.units||payload.metadata?.units||'units not specified'} · ${payload.cells.length} available cases`;$('hypercubeCaseSortLabel').hidden=state.hypercubeAnalysis.view!=='table';if(state.hypercubeAnalysis.view==='curves')renderHypercubeCurves(payload);else if(state.hypercubeAnalysis.view==='table')renderHypercubeCaseTable(payload);else renderHypercubeHeatmap(payload);renderHypercubeSelection();}
...
async function exportHypercubeVisual(format){const svgText=hypercubeVisualSvg(),filename=compareExportFilename('hypercube analysis',format),invoke=window.__TAURI_INTERNALS__?.invoke;if(format==='svg'){if(invoke){const saved=await invoke('save_visual_export',{format,content:svgText,filename,width:1600,height:1000});if(saved)notify(`Saved ${saved}.`,'success');return;}const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([svgText],{type:'image/svg+xml'}));link.download=filename;link.click();return;}const url=URL.createObjectURL(new Blob([svgText],{type:'image/svg+xml'})),image=new Image();await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=reject;image.src=url});const canvas=document.createElement('canvas');canvas.width=1600;canvas.height=1000;const context=canvas.getContext('2d');context.fillStyle='#fff';context.fillRect(0,0,1600,1000);context.drawImage(image,0,0);URL.revokeObjectURL(url);const content=canvas.toDataURL(format==='pdf'?'image/jpeg':'image/png',.94);if(invoke){const saved=await invoke('save_visual_export',{format,content,filename,width:1600,height:1000});if(saved)notify(`Saved ${saved}.`,'success');return;}if(format==='pdf')return notify('PDF export is available in the desktop app.','error');const link=document.createElement('a');link.href=content;link.download=filename;link.click();}
function exportHypercubeCsv(){return saveBackendExport('hypercube-analysis-csv').catch((error)=>notify(error.message||String(error),'error'));}
async function startHypercubeDiscovery(){if(hypercubeOperationActive())return;const payload=hypercubeAnalysisRequest();if(!payload.projectId||!payload.year)return notify('Choose a Hypercube project and year first.','error');const operation=await post('/api/hypercube-analysis/discovery/start',{projectId:payload.projectId,year:payload.year,filterField:payload.filterField,filterValues:payload.filterValues,caseIds:payload.caseIds});state.hypercubeAnalysis.discoveryOperationId=operation.id;state.hypercubeAnalysis.operationKind='discovery';state.hypercubeAnalysis.operationStartedAt=Date.now();state.hypercubeAnalysis.requestSnapshot=structuredClone(payload);$('cancelHypercubeDiscovery').hidden=false;setHypercubeAnalysisLocked(true,operation);pollHypercubeDiscovery(operation.id);}
async function pollHypercubeDiscovery(operationId=state.hypercubeAnalysis.discoveryOperationId){const id=operationId;if(!id||state.hypercubeAnalysis.discoveryOperationId!==id)return;try{const status=await request(`/api/hypercube-analysis/discovery/status?id=${encodeURIComponent(id)}`),progress=status.progress||{};if(state.hypercubeAnalysis.discoveryOperationId!==id)return;$('hypercubeDiscoveryStatus').textContent=`${status.message}${progress.total?` · ${progress.completed} of ${progress.total}`:''}`;renderHypercubeActivity(status);if(['waiting','running','cancelling'].includes(status.state)){state.hypercubeAnalysis.pollTimer=setTimeout(()=>pollHypercubeDiscovery(id),900);return;}state.hypercubeAnalysis.discoveryOperationId='';state.hypercubeAnalysis.operationKind='';$('cancelHypercubeDiscovery').hidden=true;setHypercubeAnalysisLocked(Boolean(state.hypercubeAnalysis.matrixOperationId));if(status.state==='succeeded'){state.hypercubeAnalysis.discoveryResult=status.result;renderHypercubeDiscovery(status.result);if(state.activePrimaryPage!=='hypercubePage'||state.activeHypercubeSubpage!=='hypercubeAnalyzePage')nativeNotification('Responsive-output discovery complete',status.message,{outcome:'succeeded',force:true});}else if(status.state==='failed'){notify(status.message,'error');nativeNotification('Responsive-output discovery failed',status.message,{outcome:'failed',force:true});}}catch(error){notify(error.message,'error');state.hypercubeAnalysis.discoveryOperationId='';setHypercubeAnalysisLocked(Boolean(state.hypercubeAnalysis.matrixOperationId));}}
function renderHypercubeDiscovery(result){
  const target=$('hypercubeDiscoveryResults'),items=result?.results||[],tables=[...new Set(items.map((item)=>item.table))].sort();
  target.innerHTML=`<div class="hypercube-discovery-toolbar"><label>Search outputs<input id="hypercubeDiscoverySearch" type="search" placeholder="Table or variable"></label><label>Table<select id="hypercubeDiscoveryTable"><option value="">All tables</option>${tables.map((table)=>`<option>${escapeHtml(table)}</option>`).join('')}</select></label><label>Rank by<select id="hypercubeDiscoveryRank"><option value="largestOverallShift">Largest overall shift</option><option value="largestTypicalRowShift">Largest typical row shift</option><option value="broadestChange">Broadest change</option><option value="largestExtremeChange">Largest extreme change</option><option value="geographicSpread">Geographic spread</option></select></label></div><div id="hypercubeDiscoveryTableWrap" class="table-wrap"></div>`;
  const paint=()=>{const query=$('hypercubeDiscoverySearch').value.trim().toLowerCase(),table=$('hypercubeDiscoveryTable').value,rank=$('hypercubeDiscoveryRank').value,rows=items.filter((item)=>(!table||item.table===table)&&(!query||`${item.table} ${item.variable} ${item.description||''}`.toLowerCase().includes(query))).sort((a,b)=>(Number(b[rank])||-Infinity)-(Number(a[rank])||-Infinity));$('hypercubeDiscoveryTableWrap').innerHTML=`<table><thead><tr><th>Output</th><th>Overall shift</th><th>Typical row shift</th><th>Breadth</th><th>Extreme change</th><th>Geographic spread</th><th>Coverage</th><th>Warnings</th></tr></thead><tbody>${rows.map((item)=>`<tr><td><button class="text-button" data-discovered-table="${escapeHtml(item.table)}" data-discovered-variable="${escapeHtml(item.variable)}">${escapeHtml(item.table)} / ${escapeHtml(item.variable)}</button></td><td>${number(item.largestOverallShift)}%</td><td>${item.largestTypicalRowShift==null?'—':`${number(item.largestTypicalRowShift)}%`}</td><td>${item.broadestChange==null?'—':`${number(item.broadestChange)}%`}</td><td>${item.largestExtremeChange==null?'—':number(item.largestExtremeChange)}</td><td>${item.geographicSpread==null?'—':`${number(item.geographicSpread)}%`}</td><td>${item.availableCases} / ${item.totalCases}</td><td>${['Household','Vehicle','Worker'].includes(item.table)?'Run-local IDs; aggregate distributions used':'—'}</td></tr>`).join('')}</tbody></table>`;target.querySelectorAll('[data-discovered-variable]').forEach((button)=>button.addEventListener('click',()=>{$('hypercubeAnalysisTable').value=button.dataset.discoveredTable;renderHypercubeAnalysisVariables();$('hypercubeAnalysisVariable').value=button.dataset.discoveredVariable;renderHypercubeAnalysisYears();updateHypercubeAnalysis();}));};
...
function hypercubeRunPlan(project){const jobs=(state.data?.jobs||[]).filter((job)=>job.projectId===project?.id),latest=new Map();jobs.sort((a,b)=>String(a.createdAt||'').localeCompare(String(b.createdAt||''))).forEach((job)=>latest.set(job.baseline?'baseline':job.variationId,job));const ids=project?.hypercubes?.[0]?.scenarioIds||[],hasCurrent=(id)=>(project?.resultStatuses?.[id]||[]).some((item)=>item.status==='current'),classify=(id)=>{const job=latest.get(id);if(job&&['waiting','preparing','running','exporting','stopping'].includes(job.state))return job.state;if(hasCurrent(id))return'successful';if(job&&['failed','cleanup_failed'].includes(job.state))return'failed';return'missing';},entry=(id,name,baseline=false)=>({id,name,baseline,status:classify(id),jobId:latest.get(id)?.id||''}),entries=[entry('baseline','Baseline',true),...ids.map((id)=>{const variation=(project.variations||[]).find((item)=>item.id===id);return entry(id,variation?.name||id,false)})];return{entries,counts:entries.reduce((result,item)=>(result[item.status]=(result[item.status]||0)+1,result),{}),jobs};}
function renderHypercubeRun(){const id=fillHypercubeWorkflowSelect($('hypercubeRunProject'),true),project=hypercubeWorkflowProjects().find((item)=>item.id===id),target=$('hypercubeRunCounts');if(!project){target.innerHTML='';renderRunHistoryActions();return;}const plan=hypercubeRunPlan(project),counts=plan.counts;target.innerHTML=['successful','missing','failed','waiting','running'].map((key)=>metric(key[0].toUpperCase()+key.slice(1),counts[key]||0)).join('');$('runMissingHypercube').disabled=!(counts.missing);$('retryFailedHypercube').disabled=!(counts.failed);const total=plan.entries.length,done=counts.successful||0,active=(counts.waiting||0)+(counts.preparing||0)+(counts.running||0)+(counts.exporting||0)+(counts.stopping||0),concurrency=Math.max(1,Number(state.desktop?.resources?.maxConcurrentRuns||1)),waves=Math.ceil(total/concurrency),started=plan.jobs.map((job)=>new Date(job.startedAt||'').getTime()).filter(Number.isFinite),elapsed=started.length?formatDuration(Date.now()-Math.min(...started)):'Not started';$('hypercubeBatchCard').className='panel';$('hypercubeBatchCard').innerHTML=`<div class="section-title"><div><h3>${escapeHtml(project.name)}</h3><p>${done} of ${total} complete${active?` · ${active} active or queued`:''}</p></div><span class="pill">${Math.round(done/Math.max(1,total)*100)}%</span></div><div class="metric-grid">${metric('Execution waves',waves)}${metric('Elapsed',elapsed)}${metric('Concurrency',concurrency)}${metric('Status',active?'Processing':done===total?'Complete':'Ready')}</div><progress max="${total}" value="${done}"></progress>`;$('hypercubeRunCases').innerHTML=plan.entries.map((item)=>`<button type="button" class="hypercube-case-row" data-hypercube-run-job="${escapeHtml(item.jobId)}" ${item.jobId?'':'disabled'}><span>${escapeHtml(item.name)}</span><span class="pill">${escapeHtml(item.status)}</span></button>`).join('');const history=plan.jobs;$('hypercubeRunHistory').className='run-history-scroll'+(history.length?'':' empty-state');$('hypercubeRunHistory').innerHTML=history.length?history.map((job)=>`<button class="job-card" type="button" data-hypercube-history-job="${escapeHtml(job.id)}"><strong>${escapeHtml(jobDisplayName(job))}</strong><span>${escapeHtml(job.state)} · ${escapeHtml(jobRuntime(job))}</span></button>`).join(''):'No Hypercube jobs.';document.querySelectorAll('[data-hypercube-run-job],[data-hypercube-history-job]').forEach((button)=>button.addEventListener('click',()=>showHypercubeJobLog(button.dataset.hypercubeRunJob||button.dataset.hypercubeHistoryJob)));renderRunHistoryActions();}
...
async function startHypercubePlannedRun(kind){const project=hypercubeWorkflowProjects().find((item)=>item.id===$('hypercubeRunProject').value);if(!project)return;const mode=state.data?.runtime?.adapter==='native'?'queued':state.data?.queue?.modeLock||'parallel';try{await post('/api/hypercube-run/start',{projectId:project.id,selection:kind,mode});notify(`${kind==='failed'?'Failed':'Missing'} Hypercube work was queued.`,'success');await refreshState({quiet:true});renderHypercubeRun();}catch(error){notify(error.message,'error');}}
...
[['hypercubeAnalysisHeatmapView','matrix'],['hypercubeAnalysisCurveView','curves'],['hypercubeAnalysisTableView','table']].forEach(([id,view])=>$(id).addEventListener('click',()=>{state.hypercubeAnalysis.view=view;['hypercubeAnalysisHeatmapView','hypercubeAnalysisCurveView','hypercubeAnalysisTableView'].forEach((button)=>$(button).classList.toggle('active',button===id));if(state.hypercubeAnalysis.matrix)renderHypercubeAnalysis();}));
$('openHypercubeCompare').addEventListener('click',()=>openHypercubeInCompare(false));$('compareHypercubePair').addEventListener('click',()=>openHypercubeInCompare(true));$('viewHypercubeRows').addEventListener('click',()=>openHypercubeInCompare(false));$('openHypercubeMap').addEventListener('click',openHypercubeInMap);
$('exportHypercubePng').addEventListener('click',()=>exportHypercubeVisual('png'));$('exportHypercubeSvg').addEventListener('click',()=>exportHypercubeVisual('svg'));$('exportHypercubePdf').addEventListener('click',()=>exportHypercubeVisual('pdf'));$('exportHypercubeCsv').addEventListener('click',exportHypercubeCsv);
$('exportHypercubeExcel').addEventListener('click',()=>exportArtifact('hypercube-analysis',{analysisRequest:hypercubeAnalysisRequest()}));
