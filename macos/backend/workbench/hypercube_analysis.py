...
class HypercubeCaseExportManager:
    ...
    def __init__(self, service: HypercubeAnalysisService):
        ...
        for operation_path in self.root.glob("*/operation.json"):
            ...
            staging = self.workspace.models / f".hypercube-export-{operation_path.parent.name}"
    ...
    def _run(self, operation_id: str) -> None:
        directory = self.root / operation_id
        staging = self.workspace.models / f".hypercube-export-{operation_id}"
        ...
        try:
            ...
            self._update(operation_id, state="succeeded", phase="complete", finishedAt=now_iso(), message=f"Prepared {snapshot['name']}", progress={"completed": 4, "total": 4}, artifactBytes=artifact.stat().st_size)
        except Exception as exc:
            cancelled = self._cancelled(operation_id)
            self._update(operation_id, state="cancelled" if cancelled else "failed", phase="cancelled" if cancelled else "failed", finishedAt=now_iso(), message="Hypercube case export cancelled" if cancelled else str(exc))
        finally:
            heartbeat_stop.set()
            shutil.rmtree(staging, ignore_errors=True)
